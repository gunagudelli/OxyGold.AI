...
const ShimmerBox = ({ width, height, borderRadius = 8 }) => {
  ...
  return (
    <Animated.View style={{ width, height, borderRadius, backgroundColor: '#C8972A', opacity }} />
  );
};
...
const PgHomeScreen = ({ navigation, route }) => {
    ...
    categories.forEach((cat) => {
      if (!categoryImages[cat.id]) {
        ...
        if (imageUrl) {
          ...
          getCategoryImage(cat.id).then(img => {
            if (img) setCategoryImages(prev => ({ ...prev, [cat.id]: img }));
          }).catch(() => {});
        }
      }
    });
    ...
    subCategories.forEach((subCat) => {
      if (!subCategoryImages[subCat.id]) {
        ...
        if (imageUrl) {
          ...
          getCategoryImage(subCat.id).then(img => {
            if (img) setSubCategoryImages(prev => ({ ...prev, [subCat.id]: img }));
          }).catch(() => {});
        }
      }
    });
  ...
  return (
    <PgLayout title="GoldMart" showBack={false}>
      ...
      >
        {/* ── Hero ── */}
        <Animated.View style={[styles.hero, { opacity: heroAnim, transform: [{ translateY: heroTranslate }] }]}>
          ...
          <View style={styles.heroLeft}>
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('PgProducts', { accessToken, userId })}
              activeOpacity={0.85}
            >
        ...
        {loading?.categories ? (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.catScrollPad}>
            {[1, 2, 3, 4].map(i => (
              ...
              return (
                ...
                >
                  <View style={[styles.catImgWrap, isActive && styles.catImgWrapActive]}>
                    <Image
                      source={{ uri: categoryImages[c.id] || 'https://via.placeholder.com/64?text=Gold' }}
                      style={styles.catImg}
                      onError={() => setCategoryImages(prev => ({ ...prev, [c.id]: 'https://via.placeholder.com/64?text=Gold' }))}
                    />
                    {isActive && <View style={styles.catImgOverlay} />}
                  </View>
                  <Text style={[styles.catLabel, isActive && styles.catLabelActive]}>{c?.name}</Text>
        ...
        {!loading?.subCategories && subCategories?.length > 0 && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.subScrollPad}>
            {subCategories?.map((s) => {
              ...
              return (
                ...
                >
                  <Image
                    source={{ uri: subCategoryImages[s.id] || 'https://via.placeholder.com/22?text=Gold' }}
                    style={styles.subChipImg}
                    onError={() => setSubCategoryImages(prev => ({ ...prev, [s.id]: 'https://via.placeholder.com/22?text=Gold' }))}
                  />
                  <Text style={[styles.subChipText, isActive && styles.subChipTextActive]}>{s?.name}</Text>
                </TouchableOpacity>
              );
            })}
          </ScrollView>
        ...
        <SectionHeader
          ...
          onViewAll={() => navigation.navigate('PgProducts', { categoryId: activeCategory, subCategoryId: activeSubCat, accessToken, userId })}
        />
            ...
            {products?.slice(0, 6)?.map((item) => (
              <View key={item?.id} style={styles.gridItem}>
                <ProductCard
                  ...
                  onPress={() => navigation.navigate('PgProductDetails', { 
                    productId: item?.id, 
                    product: item,
                    accessToken,
                    userId 
        ...
        {products?.length > 6 && (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('PgProducts', { categoryId: activeCategory, subCategoryId: activeSubCat, accessToken, userId })}
            activeOpacity={0.85}
          >
            <Text style={styles.loadMoreText}>View All {products?.length} Products</Text>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('Dashboard', { accessToken, userId })}
          activeOpacity={0.87}
        >
...
const styles = StyleSheet.create({
  ...
  hero: {
    ...
    backgroundColor: C.heroBase, borderRadius: 24,
  ...
  heroCircle1: {
    ...
    backgroundColor: 'rgba(212,175,55,0.10)', top: -80, right: -60,
  },
  heroCircle2: {
    position: 'absolute', width: 120, height: 120, borderRadius: 60,
    backgroundColor: 'rgba(212,175,55,0.05)', bottom: -50, left: 80,
  },
  heroLeft: { flex: 1, zIndex: 1 },
  heroBadgePill: {
    flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start',
    backgroundColor: 'rgba(212,175,55,0.15)', borderRadius: 20,
    paddingHorizontal: 10, paddingVertical: 5, marginBottom: 14,
    borderWidth: 1, borderColor: 'rgba(212,175,55,0.30)',
  },
  heroBadgeDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: C.goldBright, marginRight: 6 },
  heroBadgePillText: { fontSize: 9, fontWeight: '800', color: C.goldBright, letterSpacing: 1.2 },
  heroTitle: {
    fontSize: 26, fontWeight: '900', color: C.heroText,
    lineHeight: 32, marginBottom: 8, letterSpacing: -0.3,
  },
  heroSubtitle: { fontSize: 12, color: C.heroTextSec, marginBottom: 20, lineHeight: 18 },
  heroShopBtn: {
    flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start',
    backgroundColor: C.goldBright, borderRadius: 12,
    paddingHorizontal: 18, paddingVertical: 11, gap: 6,
  },
  ...
  heroCoin: {
    ...
    borderWidth: 2, borderColor: C.goldBright,
    justifyContent: 'center', alignItems: 'center',
    shadowColor: '#D4AF37', shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.5, shadowRadius: 14, elevation: 10,
  },
  heroCoinKarat: { fontSize: 20, fontWeight: '900', color: C.goldBright, lineHeight: 22 },
  heroCoinDivider: { width: 30, height: 1, backgroundColor: 'rgba(212,175,55,0.45)', marginVertical: 3 },
  heroCoinPurity: { fontSize: 11, fontWeight: '700', color: 'rgba(240,204,90,0.75)', lineHeight: 14 },
  heroCoinPure: { fontSize: 8, fontWeight: '600', color: 'rgba(245,237,208,0.35)', letterSpacing: 1.5 },
  heroCoinLabel: { fontSize: 9, fontWeight: '700', color: 'rgba(245,237,208,0.35)', letterSpacing: 2, marginTop: 6 },
  ...
  trustLabel: { fontSize: 10, fontWeight: '700', color: C.textSec, textAlign: 'center', lineHeight: 15 },
  trustDivider: { width: 1, height: 32, backgroundColor: C.border },

  sectionHeader: {
    flexDirection: 'row', justifyContent: 'space-between',
    alignItems: 'center', marginHorizontal: 16, marginBottom: 14,
  },
  sectionTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  sectionAccent: { width: 3, height: 18, borderRadius: 2, backgroundColor: C.gold },
  sectionTitle: { fontSize: 16, fontWeight: '800', color: C.textPri, letterSpacing: -0.2 },
  sectionCount: {
    fontSize: 11, fontWeight: '600', color: C.textTer,
    backgroundColor: C.surfaceAlt, paddingHorizontal: 7,
  ...
  catLabel: { fontSize: 11, fontWeight: '600', color: C.textSec, textAlign: 'center', maxWidth: 68 },
  catLabelActive: { color: C.gold, fontWeight: '800' },
  catActiveDot: { width: 4, height: 4, borderRadius: 2, backgroundColor: C.gold, marginTop: 4 },
  ...
  loadMoreBtn: {
    ...
    paddingVertical: 15, gap: 4,
    borderWidth: 1.5, borderColor: C.goldDimBorder,
    shadowColor: '#000', shadowOffset: { width: 0, height: 1 },
  ...
  emptyTitle: { fontSize: 15, fontWeight: '700', color: C.textSec, marginBottom: 4 },
  emptySubtitle: { fontSize: 12, color: C.textTer },

  digitalBanner: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    marginHorizontal: 16, marginTop: 4,
    backgroundColor: '#1A1200', borderRadius: 18,
    padding: 18,
  ...
  digitalBannerGlow: {
    ...
    backgroundColor: 'rgba(212,175,55,0.08)', top: -50, right: 10,
  },
  ...
  digitalIconText: { fontSize: 12, fontWeight: '900', color: C.goldBright, letterSpacing: 0.5 },
  digitalTitle: { fontSize: 14, fontWeight: '800', color: C.goldBright, marginBottom: 3 },
  digitalSubtitle: { fontSize: 11, color: 'rgba(245,237,208,0.50)' },
  digitalCTA: {
    backgroundColor: C.goldBright, borderRadius: 10,
    paddingHorizontal: 14, paddingVertical: 9,
  },
