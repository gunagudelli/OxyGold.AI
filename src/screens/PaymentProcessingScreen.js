...
import {
  ...
  Animated, Easing, Alert, BackHandler, TouchableOpacity, ActivityIndicator,
} from 'react-native';
...
// ── Polling View ──────────────────────────────────────────────────────────────
const PollingView = ({ amount, grams, method, statusText, pollAttempt, spinAnim, pulseAnim }) => {
  ...
  return (
    <View style={s.center}>
      ...
      {pollAttempt > 1 && (
        <Text style={s.attemptText}>Checking... {pollAttempt}/{MAX_ATTEMPTS}</Text>
      )}
      <View style={s.infoCard}>
        <View style={s.infoRow}>
          <Text style={s.infoLabel}>Amount</Text>
          <Text style={s.infoValue}>₹{Number(amount).toLocaleString('en-IN', { maximumFractionDigits: 2 })}</Text>
        </View>
        <View style={s.infoRow}>
          <Text style={s.infoLabel}>Gold</Text>
          <Text style={[s.infoValue, { color: '#f0bb3a' }]}>{Number(grams).toFixed(4)} grams</Text>
        </View>
        <View style={s.infoRow}>
...
const PaymentProcessingScreen = ({ navigation, route }) => {
  ...
  const isValidSession = cleanSessionId.startsWith('session_') && cleanSessionId.length > 20;
  ...
  const startPolling = () => {
    ...
    const poll = async () => {
      ...
      else if (attempt.current <= 7)  setStatusText('Confirming with payment gateway...');
      else if (attempt.current <= 12) setStatusText('Allocating gold to your vault...');
      ...
      try {
        ...
        if (attempt.current >= MAX_ATTEMPTS) {
          ...
          Alert.alert(
            ...
            [{ text: 'Check Portfolio', onPress: () => navigation.navigate('Dashboard') }]
          );
        }
      } catch {
    ...
    return (
      <SafeAreaView style={s.root} edges={['top', 'bottom']}>
        ...
        <View style={s.center}>
          ...
          <TouchableOpacity style={s.errorBtn} onPress={() => navigation.navigate('Dashboard')}>
  ...
  return (
    <SafeAreaView style={s.root} edges={['top', 'bottom']}>
      ...
      {phase === 'checkout' ? (
            ...
            <TouchableOpacity
              onPress={() => Alert.alert(
                ...
                [
                  ...
                  { text: 'Yes, Cancel', style: 'destructive', onPress: () => navigation.navigate('Dashboard') },
                ]
              )}
            >
        ...
        <PollingView
          amount={amount} grams={grams} method={method}
          statusText={statusText} pollAttempt={pollAttempt}
          spinAnim={spinAnim} pulseAnim={pulseAnim}
        />
      )}
    </SafeAreaView>
  );
...
const s = StyleSheet.create({
  ...
  webviewHeader: {
    ...
    paddingHorizontal: 20, paddingVertical: 14, backgroundColor: '#0d1f3c',
  },
  ...
  loadingOverlay: {
    ...
    backgroundColor: '#0d1f3c', justifyContent: 'center', alignItems: 'center', gap: 16,
  },
  ...
  spinnerArc:   { width: 110, height: 110, borderRadius: 55, borderWidth: 4, borderColor: 'transparent', borderTopColor: '#f0bb3a', borderRightColor: 'rgba(240,187,58,0.3)' },
  spinnerInner: { width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(240,187,58,0.1)', borderWidth: 1, borderColor: 'rgba(240,187,58,0.25)', justifyContent: 'center', alignItems: 'center' },
  ...
  infoCard:  { width: '100%', backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 18, gap: 12, borderWidth: 1, borderColor: 'rgba(240,187,58,0.15)' },
  ...
  errorBtn:     { marginTop: 24, paddingVertical: 12, paddingHorizontal: 32, backgroundColor: '#f0bb3a', borderRadius: 8 },
