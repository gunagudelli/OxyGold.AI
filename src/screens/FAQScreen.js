...
import {
  ...
  StyleSheet, StatusBar, Animated, LayoutAnimation, Platform, UIManager,
} from 'react-native';
    ...
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
        ...
        <LinearGradient colors={['#0d1f3c', '#112347']} style={s.hero}>
          ...
          <Text style={s.heroTitle}>Frequently Asked{'\n'}<Text style={s.heroGold}>Questions</Text></Text>
          <Text style={s.heroSub}>Everything you need to know about Digital Gold</Text>
        </LinearGradient>
        ...
        <View style={s.chipsWrap}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.chips}>
            ...
            {FAQS.map(c => (
              ...
              >
                <Text style={[s.chipText, activeCategory === c.category && s.chipTextActive]}>
                  {c.category}
                </Text>
              </TouchableOpacity>
            ))}
            ...
            <View key={cat.category} style={s.section}>
              ...
              <View style={s.card}>
                {cat.items.map((item, i) => (
                  <FaqItem key={i} item={item} isLast={i === cat.items.length - 1} />
                ))}
              </View>
            </View>
          ))}
        ...
        <LinearGradient colors={['#112347', '#0d1f3c']} style={s.cta}>
          ...
          <Text style={s.ctaSub}>Our support team is available 24/7 to help you</Text>
...
const s = StyleSheet.create({
  ...
  heroLabel: { fontSize: 10, fontWeight: '600', color: '#f0bb3a', letterSpacing: 1.2, textTransform: 'uppercase', marginBottom: 8 },
  heroTitle: { fontSize: 26, fontWeight: '800', color: '#fff', lineHeight: 34, marginBottom: 8 },
  ...
  chipActive:     { backgroundColor: 'rgba(240,187,58,0.15)', borderColor: 'rgba(240,187,58,0.5)' },
  chipText:       { fontSize: 12, fontWeight: '500', color: 'rgba(255,255,255,0.55)' },
  ...
  catHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 10, paddingHorizontal: 4 },
  catTitle:  { fontSize: 15, fontWeight: '700', color: '#fff', flex: 1 },
  catCount:  {
    fontSize: 11, fontWeight: '700', color: '#f0bb3a',
  ...
  // Card
  card: {
    backgroundColor: '#112347', borderRadius: 14,
    borderWidth: 1, borderColor: 'rgba(42,78,158,0.3)',
    overflow: 'hidden',
  },

  // FAQ item
  faqItem:     { borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.06)' },
  ...
  faqQText:       { fontSize: 14, fontWeight: '600', color: '#fff', flex: 1, lineHeight: 20 },
  faqChevron:     { fontSize: 22, color: 'rgba(255,255,255,0.35)', transform: [{ rotate: '0deg' }] },
  ...
  // CTA
  cta:       { marginHorizontal: 16, borderRadius: 14, padding: 24, alignItems: 'center', marginTop: 8 },
  ctaTitle:  { fontSize: 18, fontWeight: '700', color: '#fff', marginBottom: 6 },
  ctaSub:    { fontSize: 13, color: 'rgba(255,255,255,0.55)', marginBottom: 20, textAlign: 'center' },
  ctaBtn:    { backgroundColor: '#f0bb3a', paddingHorizontal: 28, paddingVertical: 12, borderRadius: 8 },
