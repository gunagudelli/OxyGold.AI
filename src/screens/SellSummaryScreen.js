...
import {
  ...
  Alert, ActivityIndicator, StatusBar, BackHandler,
} from 'react-native';
...
export default function SellSummaryScreen({ navigation, route }) {
  ...
  useEffect(() => {
    const sub = BackHandler.addEventListener('hardwareBackPress', () => sellLoading);
    return () => sub.remove();
  }, [sellLoading]);
  ...
  const handleConfirm = async () => {
    ...
    if (!bankDetails) {
      Alert.alert('Bank Required', 'Please add a bank account before proceeding.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Transaction Failed', e.message || 'Could not process sell. Please try again.');
    } finally {
  ...
  return (
    <SafeAreaView style={s.root} edges={['top', 'bottom']}>
      ...
      {/* Header */}
      <View style={s.header}>
        <TouchableOpacity style={s.backBtn} onPress={() => navigation.goBack()} disabled={sellLoading}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
        ...
        {/* Timer Banner */}
        <View style={[s.timerBanner, isUrgent ? s.timerUrgent : s.timerOk, !isPriceLocked && s.timerExpired]}>
          <View style={s.timerRow}>
            <Text style={s.timerIcon}>{isPriceLocked ? '🔒' : '⚠️'}</Text>
            <Text style={[s.timerText, isUrgent && !isPriceLocked ? s.timerTextExpired : isUrgent ? s.timerTextUrgent : s.timerTextOk]}>
              {isPriceLocked ? 'Price locked — expires in' : 'Price lock expired'}
            </Text>
            {isPriceLocked && (
              <View style={[s.timerPill, isUrgent ? s.timerPillUrgent : s.timerPillOk]}>
                <Text style={[s.timerPillText, isUrgent ? s.timerPillTextUrgent : s.timerPillTextOk]}>
                  {mins}:{secs}
                </Text>
              </View>
            )}
          ...
          {isPriceLocked && (
            <View style={s.timerBarBg}>
              <View style={[s.timerBarFill, { width: `${timerPct}%`, backgroundColor: isUrgent ? '#d97706' : '#1a3060' }]} />
            </View>
          )}
        </View>
        ...
        <View style={s.card}>
          ...
          <Row label="Sell Rate (Locked)" value={`₹${parseFloat(sellRate).toLocaleString('en-IN', { maximumFractionDigits: 2 })} / gram`} gold />
          <Row label="Quantity"           value={`${parseFloat(grams).toFixed(4)} grams`} />
          <View style={s.divider} />
          <Row label="You Will Receive"   value={`₹${parseFloat(amount).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`} gold />
          <Text style={s.gstNote}>* Selling price is GST-adjusted as per regulations</Text>
        </View>
        ...
        <View style={s.card}>
          ...
          {bankLoading ? (
            ...
            <>
              ...
              {bankDetails.ifsc && <Row label="IFSC Code" value={bankDetails.ifsc} />}
              {bankDetails.branch && <Row label="Branch" value={`${bankDetails.branch}${bankDetails.city ? ', ' + bankDetails.city : ''}`} />}
              <TouchableOpacity
                style={s.changeBtn}
                onPress={() => navigation.navigate('BankAccount', { returnTo: 'SellSummary', ...route.params })}
              >
        ...
        {/* Terms */}
        <TouchableOpacity style={s.termsRow} onPress={() => setIsAccepted(v => !v)} activeOpacity={0.7}>
        ...
        <View style={s.secRow}>
          ...
          <Text style={s.secText}>256-bit SSL encrypted · T+1 settlement · TDS applicable above ₹50,000</Text>
        </View>
      ...
      <View style={s.footer}>
        {isPriceLocked ? (
          <TouchableOpacity
            style={[s.confirmBtn, (!isAccepted || !bankDetails || sellLoading) && s.confirmBtnDisabled]}
            onPress={handleConfirm}
            disabled={!isAccepted || !bankDetails || sellLoading || !isPriceLocked}
            activeOpacity={0.85}
          >
            {sellLoading
              ? <ActivityIndicator color="#0d1f3c" size="small" />
              : <Text style={s.confirmBtnText}>
                  {bankDetails ? 'Confirm & Sell Gold →' : 'Add Bank Account First'}
                </Text>
            }
          </TouchableOpacity>
        ) : (
          <TouchableOpacity style={s.refreshBtn} onPress={() => navigation.navigate('SellGold')}>
...
const s = StyleSheet.create({
  ...
  timerBanner:  { borderRadius: 10, padding: 12, marginBottom: 16, borderWidth: 1 },
  timerOk:      { backgroundColor: '#fff', borderColor: '#e8ecf0' },
  timerUrgent:  { backgroundColor: '#fffbeb', borderColor: 'rgba(217,119,6,0.25)' },
  timerExpired: { backgroundColor: '#fef2f2', borderColor: 'rgba(220,38,38,0.2)' },
  ...
  timerBarBg:   { height: 3, backgroundColor: 'rgba(0,0,0,0.06)', borderRadius: 99, overflow: 'hidden' },
  ...
  cardTitle: { fontSize: 13, fontWeight: '700', color: '#333', marginBottom: 14, textTransform: 'uppercase', letterSpacing: 0.5 },
  row:       { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#f4f5f7' },
  ...
  bankError:     { backgroundColor: '#fef2f2', borderRadius: 8, padding: 12, alignItems: 'center', gap: 8 },
  bankErrorText: { fontSize: 13, color: '#dc2626', textAlign: 'center' },
  retryBtn:      { paddingHorizontal: 16, paddingVertical: 6, borderRadius: 6, backgroundColor: '#f4f5f7', borderWidth: 1, borderColor: '#e4e7eb' },
  retryBtnText:  { fontSize: 12, color: '#1a3060', fontWeight: '600' },

  changeBtn:     { marginTop: 12, alignSelf: 'center', paddingHorizontal: 16, paddingVertical: 7, borderRadius: 6, backgroundColor: '#f4f5f7', borderWidth: 1, borderColor: '#e4e7eb' },
  ...
  addBankBtn:    { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8, borderWidth: 1, borderColor: '#464B8B' },
  ...
  checkbox:    { width: 18, height: 18, borderRadius: 4, borderWidth: 1.5, borderColor: '#ccc', justifyContent: 'center', alignItems: 'center' },
  ...
  secRow:  { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#f0fdf4', borderRadius: 8, padding: 10, borderWidth: 1, borderColor: '#10B981' },
  secText: { fontSize: 11, color: '#10B981', flex: 1 },

  footer:             { paddingHorizontal: 20, paddingBottom: 32, paddingTop: 12, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#f0f0f0' },
  confirmBtn:         { paddingVertical: 15, borderRadius: 10, backgroundColor: '#f0bb3a', alignItems: 'center' },
  confirmBtnDisabled: { opacity: 0.55 },
  confirmBtnText:     { fontSize: 15, fontWeight: '700', color: '#0d1f3c' },
  refreshBtn:         { paddingVertical: 15, borderRadius: 10, backgroundColor: '#1a3060', alignItems: 'center' },
