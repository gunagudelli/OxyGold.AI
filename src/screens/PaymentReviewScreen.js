...
const PaymentReviewScreen = ({ navigation, route }) => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((t) => {
        if (t <= 1) {
          ...
          Alert.alert(
            "Price Expired",
            "The locked price has expired. Please start a new order.",
  ...
  const callback = {
    ...
    onError: (error, orderID) => {
      ...
      Alert.alert(
        "Payment Failed",
        error?.getMessage?.() || "Payment could not be processed",
      );
      setLoading(false);
    },
  };
    ...
    <SafeAreaView style={s.root} edges={["top", "bottom"]}>
      ...
      >
          ...
          <View style={s.lockBarBg}>
            <View
              style={[
                ...
                {
                  width: `${lockPct}%`,
                  backgroundColor: lockPct > 40 ? "#f0bb3a" : "#ef4444",
                },
              ]}
            />
          </View>
        ...
        <View style={s.card}>
          ...
          <Row
            label="Rate per gram"
            value={`₹${pergramPrice.toLocaleString("en-IN", { maximumFractionDigits: 2 })}`}
          />
          <Row
            label="Gold Value"
            value={`₹${goldValue.toLocaleString("en-IN", { maximumFractionDigits: 2 })}`}
          />
          ...
          <Row
            label="Total Payable"
            value={`₹${totalPayable.toLocaleString("en-IN", { maximumFractionDigits: 2 })}`}
            bold
            gold
          />
        </View>
