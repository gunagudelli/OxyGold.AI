...
const DigitalGoldScreen = ({ navigation, route }) => {
  ...
  const handleBuyGold = async () => {
    ...
    try {
      ...
      const preview = await previewBuy({
        userId: uid, purchaseType, amount: sendAmount,
        grams: sendGrams, pergramBuyingPrice: goldRate,
      });
      if (!preview) throw new Error('Failed to get order preview. Please try again.');
      navigation.navigate('PaymentReview', { preview, buyMode, goldRate });
    } catch (e) {
  ...
  const gramsPreview = amount && !isNaN(parseFloat(amount)) && buyMode === 'rupees'
    ? (parseFloat(amount) / goldRate).toFixed(4) : null;
  const rupeesPreview = amount && !isNaN(parseFloat(amount)) && buyMode === 'grams'
    ? (parseFloat(amount) * goldRate).toLocaleString('en-IN', { maximumFractionDigits: 0 }) : null;
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      ...
      <ScrollView
        ...
        showsVerticalScrollIndicator={false}
        bounces={false}
        contentContainerStyle={[s.scroll, { paddingBottom: keyboardOffset > 0 ? keyboardOffset + 24 : 40 }]}
        keyboardShouldPersistTaps="handled"
      >
        ...
        >
          ...
          <View style={s.heroPortfolio}>
            <View>
              ...
              {portfolioLoading
                ? <ActivityIndicator color={C.goldMid} size="small" style={{ marginTop: 10 }} />
                : (
                  <>
                    <Text style={s.heroPortValue}>
                      ₹{currentValue.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                    </Text>
                    <Text style={s.heroPortGrams}>{goldBalance.toFixed(4)} grams</Text>
                  </>
                )
              }
            </View>
            {!portfolioLoading && (
              <View style={[s.heroGainTag, !isProfit && s.heroGainTagRed]}>
                ...
                <Text style={[s.heroGainAmt, !isProfit && s.heroGainAmtRed]}>
                  ₹{Math.abs(totalGain).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                </Text>
              </View>
            )}
          </View>
        ...
        <View style={s.savingsCard}>
          <View style={s.savingsLeft}>
            ...
            <Text style={s.savingsGrams}>
              {goldBalance.toFixed(4)}<Text style={s.savingsUnit}> g</Text>
            </Text>
          ...
          {goldBalance > 0 && (
            <TouchableOpacity onPress={() => navigation.navigate('SellGold')} activeOpacity={0.85} style={s.sellBtn}>
              <Text style={s.sellBtnLabel}>Sell Gold</Text>
              <Text style={s.sellBtnRate}>@ ₹{sellPrice.toLocaleString('en-IN', { maximumFractionDigits: 0 })}/g</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        <View style={s.card}>
          ...
          <View style={s.toggle}>
            {['rupees', 'grams'].map((mode) => (
              <TouchableOpacity
                key={mode}
                onPress={() => { setBuyMode(mode); setAmount(''); }}
                style={[s.toggleTab, buyMode === mode && s.toggleTabActive]}
                activeOpacity={0.8}
              >
                <Text style={[s.toggleTabText, buyMode === mode && s.toggleTabTextActive]}>
          ...
          {(gramsPreview || rupeesPreview) ? (
            ...
            <Text style={s.inputHint}>
              {buyMode === 'rupees' ? 'Minimum purchase ₹100  ·  Incl. 3% GST' : 'Live rate  ·  Incl. 3% GST'}
            </Text>
          )}
          ...
          {buyMode === 'rupees' && (
            <View style={s.chips}>
              {[['100','₹100'],['500','₹500'],['1000','₹1K'],['10000','₹10K']].map(([val, label]) => (
                <TouchableOpacity
                  key={val}
        ...
        <View style={s.section}>
          ...
          <View style={s.twoCol}>
            <TouchableOpacity style={s.featureCard} activeOpacity={0.8}>
              <LinearGradient colors={['#F5F4FF','#ECEAFF']} style={s.featureCardInner}>
          ...
          <View style={s.txnCard}>
            {txnLoading ? (
              ...
              transactions.map((txn, i) => {
                ...
                return (
                  <View key={txn.id} style={[s.txnRow, i < transactions.length - 1 && s.txnBorder]}>
                    <View style={[s.txnDot, isBuy ? s.txnDotBuy : s.txnDotSell]}>
                      <Text style={{ fontSize: 16, fontWeight: '700', color: isBuy ? C.green : C.red }}>
                        {isBuy ? '↑' : '↓'}
                      </Text>
                    </View>
                    <View style={s.txnMid}>
                    ...
                    <View style={s.txnRight}>
                      <Text style={s.txnAmt}>₹{txn.amount.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</Text>
...
const s = StyleSheet.create({
  ...
  backBtn: {
    ...
    borderWidth: 1, borderColor: '#EAE8E2',
    shadowColor: 'rgba(28,35,64,0.08)', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 1, shadowRadius: 6, elevation: 2,
  },
  backBtnText: { fontSize: 28, lineHeight: 32, color: '#1C2340', fontWeight: '300', marginTop: -2 },
  headerTitle: { fontSize: 17, fontWeight: '700', color: '#1C2340', letterSpacing: 0.2 },
  liveChip: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    backgroundColor: '#ECFDF5', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 7,
    borderWidth: 1, borderColor: '#A7F3D0',
  },
  liveDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: '#0E9F6E' },
  liveLabel: { fontSize: 11, fontWeight: '700', color: '#0E9F6E', letterSpacing: 0.8 },
  ...
  heroPriceRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 },
  heroPurity: { fontSize: 11, fontWeight: '600', color: 'rgba(255,255,255,0.45)', letterSpacing: 1, marginBottom: 8 },
  heroPrice: { fontSize: 34, fontWeight: '800', color: '#E8C97A', letterSpacing: -1, marginBottom: 4 },
  ...
  heroPortfolio: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' },
  heroPortLabel: { fontSize: 10, fontWeight: '700', color: 'rgba(255,255,255,0.35)', letterSpacing: 1.5, marginBottom: 8 },
  heroPortValue: { fontSize: 28, fontWeight: '800', color: '#FFFFFF', letterSpacing: -0.5, marginBottom: 4 },
  heroPortGrams: { fontSize: 13, color: 'rgba(255,255,255,0.38)' },
  heroGainTag: {
    backgroundColor: 'rgba(14,159,110,0.15)', borderRadius: 12,
    borderWidth: 1, borderColor: 'rgba(14,159,110,0.25)',
    paddingHorizontal: 14, paddingVertical: 10, alignItems: 'center', minWidth: 90,
  },
  heroGainTagRed: { backgroundColor: 'rgba(224,36,36,0.12)', borderColor: 'rgba(224,36,36,0.22)' },
  heroGainPct: { fontSize: 13, fontWeight: '700', color: '#0E9F6E', marginBottom: 3 },
  ...
  savingsCard: {
    ...
    borderRadius: 18, padding: 18, borderWidth: 1, borderColor: '#EAE8E2',
    shadowColor: 'rgba(28,35,64,0.08)', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 1, shadowRadius: 8, elevation: 3,
  },
  savingsLeft: {},
  savingsLabel: { fontSize: 10, fontWeight: '700', color: '#8891AF', letterSpacing: 1.2, marginBottom: 6 },
  savingsGrams: { fontSize: 24, fontWeight: '800', color: '#C8952A', letterSpacing: -0.5, marginBottom: 4 },
  ...
  sellBtn: {
    ...
    borderColor: '#FECACA', paddingHorizontal: 16, paddingVertical: 12, alignItems: 'center',
  },
  sellBtnLabel: { fontSize: 13, fontWeight: '700', color: '#E02424', marginBottom: 3 },
  sellBtnRate: { fontSize: 11, color: '#F87171', fontWeight: '500' },

  // Card
  card: {
    backgroundColor: '#FFFFFF', marginHorizontal: 16, marginBottom: 16,
    borderRadius: 22, padding: 20, borderWidth: 1, borderColor: '#EAE8E2',
    shadowColor: 'rgba(28,35,64,0.08)', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 1, shadowRadius: 8, elevation: 3,
  },
  cardTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 },
  cardTitle: { fontSize: 20, fontWeight: '800', color: '#1C2340', letterSpacing: -0.3 },
  ratePill: {
    backgroundColor: '#F5ECD7', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 5,
  ...
  // Toggle
  toggle: {
    flexDirection: 'row', backgroundColor: '#F7F6F3', borderRadius: 14,
    padding: 4, marginBottom: 18, borderWidth: 1, borderColor: '#EAE8E2',
  },
  toggleTab: { flex: 1, paddingVertical: 11, alignItems: 'center', borderRadius: 10 },
  toggleTabActive: {
    backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#EAE8E2',
  ...
  inputBox: {
    ...
    backgroundColor: '#F7F6F3', borderRadius: 14, borderWidth: 1.5,
    borderColor: '#EAE8E2', paddingHorizontal: 18, marginBottom: 12,
  },
  inputSymbol: { fontSize: 26, fontWeight: '700', color: '#C8952A', marginRight: 6 },
  input: { flex: 1, fontSize: 30, fontWeight: '700', color: '#1C2340', paddingVertical: 16 },
  equivRow: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    backgroundColor: '#F5ECD7', borderRadius: 10,
  ...
  chip: {
    ...
    borderWidth: 1, borderColor: '#EAE8E2', backgroundColor: '#F7F6F3',
  },
  ...
  buyBtn: {
    ...
    shadowColor: 'rgba(200,149,42,0.35)', shadowOffset: { width: 0, height: 6 },
    shadowOpacity: 1, shadowRadius: 14, elevation: 6,
  },
  buyBtnGrad: { paddingVertical: 17, alignItems: 'center', borderRadius: 14 },
  buyBtnText: { fontSize: 16, fontWeight: '800', color: '#FFFFFF', letterSpacing: 0.2 },

  // Section
  section: { marginBottom: 20 },
  sectionTitle: { fontSize: 17, fontWeight: '700', color: '#1C2340', marginHorizontal: 20, marginBottom: 12 },
  sectionRowHeader: {
  ...
  featureCard: {
    flex: 1, borderRadius: 18, overflow: 'hidden', borderWidth: 1, borderColor: '#EAE8E2',
    shadowColor: 'rgba(28,35,64,0.06)', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 1, shadowRadius: 6, elevation: 2,
  },
  featureCardInner: { alignItems: 'center', paddingVertical: 22, paddingHorizontal: 12 },
  featureEmojiWrap: { position: 'relative', marginBottom: 2 },
  featureEmoji: { fontSize: 34, marginBottom: 10 },
  newTag: { position: 'absolute', top: -4, right: -14, backgroundColor: '#E02424', borderRadius: 6, paddingHorizontal: 5, paddingVertical: 2 },
  newTagText: { fontSize: 8, fontWeight: '800', color: '#FFF', letterSpacing: 0.5 },
  featureTitle: { fontSize: 14, fontWeight: '700', color: '#1C2340', marginBottom: 4, textAlign: 'center' },
  featureSub: { fontSize: 12, color: '#8891AF', textAlign: 'center' },

  // Jewellery
  jewCard: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: '#FFFFFF', marginHorizontal: 16, borderRadius: 18, padding: 16,
    borderWidth: 1, borderColor: '#EAE8E2',
    shadowColor: 'rgba(28,35,64,0.06)', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 1, shadowRadius: 6, elevation: 2,
  },
  jewIconBox: {
    ...
    backgroundColor: '#F5ECD7', justifyContent: 'center', alignItems: 'center',
    marginRight: 14, borderWidth: 1, borderColor: '#E8C97A',
  },
  jewEmoji: { fontSize: 22 },
  jewContent: { flex: 1 },
  jewTitle: { fontSize: 15, fontWeight: '700', color: '#1C2340', marginBottom: 3 },
  ...
  // Transactions
  txnCard: {
    backgroundColor: '#FFFFFF', marginHorizontal: 16, borderRadius: 18,
    borderWidth: 1, borderColor: '#EAE8E2', overflow: 'hidden',
    shadowColor: 'rgba(28,35,64,0.06)', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 1, shadowRadius: 6, elevation: 2,
  },
  ...
  txnRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 },
  txnBorder: { borderBottomWidth: 1, borderBottomColor: '#F0EEE9' },
  txnDot: { width: 38, height: 38, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginRight: 14 },
  txnDotBuy: { backgroundColor: '#ECFDF5' },
  txnDotSell: { backgroundColor: '#FEF2F2' },
  txnMid: { flex: 1 },
  txnType: { fontSize: 14, fontWeight: '600', color: '#1C2340', marginBottom: 3 },
  txnDate: { fontSize: 12, color: '#8891AF' },
  txnRight: { alignItems: 'flex-end' },
  txnAmt: { fontSize: 14, fontWeight: '700', color: '#1C2340', marginBottom: 3 },
...
pgBtnArrow: {
  ...
  panAlert: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: '#FFFBEB', marginHorizontal: 16, marginTop: 4,
    borderRadius: 18, padding: 16, borderWidth: 1, borderColor: '#FDE68A',
  },
  panAlertIcon: { fontSize: 22, marginRight: 12 },
  panAlertContent: { flex: 1 },
  panAlertTitle: { fontSize: 14, fontWeight: '700', color: '#92400E', marginBottom: 3 },
  panAlertSub: { fontSize: 12, color: '#B45309' },
  panBtn: {
    backgroundColor: '#C8952A', borderRadius: 20, paddingHorizontal: 18, paddingVertical: 9,
    shadowColor: 'rgba(200,149,42,0.3)', shadowOffset: { width: 0, height: 3 },
